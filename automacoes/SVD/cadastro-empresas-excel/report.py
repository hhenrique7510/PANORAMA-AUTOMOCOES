"""Geração de saídas — CSV, Markdown e histórico cumulativo (empresas)."""
...
FIELDS = ["razao_social", "doc", "tipo_pessoa", "status", "motivo", "screenshot"]
...
def write_markdown(items: list[dict], path: Path, *, dry_run: bool = False) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    titulo = "Cadastro de Empresas via Excel (DRY-RUN)" if dry_run else "Cadastro de Empresas via Excel"
    ...
    # uma seção por status presente (ordem estável)
    ordem = ["cadastrada", "dry_run", "ja_cadastrada", "erro"]
    presentes = ordem + [s for s in sorted(stats) if s not in ordem]
    for status in presentes:
        rows = [it for it in items if it.get("status") == status]
        if not rows:
            continue
        lines.append(f"## {status} ({len(rows)})")
        lines.append("")
        lines.append("| Razão Social | CPF/CNPJ | Tipo | Motivo |")
        lines.append("|---|---|---|---|")
        for it in rows:
            lines.append(
                f"| {it.get('razao_social','')[:45]} "
                f"| {it.get('doc','')} "
                f"| {it.get('tipo_pessoa','')} "
                f"| {it.get('motivo','')[:80]} |"
            )
        lines.append("")
...
def write_history_md(history_csv: Path, history_md: Path) -> None:
    ...
    for run_id in sorted(runs.keys(), reverse=True):
        ...
        lines.append("")
        lines.append("| Status | Razão Social | CPF/CNPJ | Motivo |")
        lines.append("|---|---|---|---|")
        for r in rows:
            lines.append(
                f"| {r['status']} | {r.get('razao_social','')[:40]} "
                f"| {r.get('doc','')} | {r.get('motivo','')[:80]} |"
            )
        lines.append("")
